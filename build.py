...
def build_flutter_dmg(version, features):
    ...
    system2('cp -rf ../target/release/service ./build/macos/Build/Products/Release/airdesk.app/Contents/MacOS/')
    '''
    system2(
...
def build_flutter_windows(version, features, skip_portable_pack):
    ...
    system2(
        f'python3 ./generate.py -f ../../{flutter_build_dir_2} -o . -e ../../{flutter_build_dir_2}/airdesk.exe')
    os.chdir('../..')
    if os.path.exists('./airdesk_portable.exe'):
        os.replace('./target/release/airdesk-portable-packer.exe',
                   './airdesk_portable.exe')
    else:
        os.rename('./target/release/airdesk-portable-packer.exe',
                  './airdesk_portable.exe')
    print(
        f'output location: {os.path.abspath(os.curdir)}/airdesk_portable.exe')
    # "-qs.exe" suffix is required so RustDesk's own is_quick_support_exe() check
    # (core_main.rs) recognizes this portable build and auto-elevates for admin
    # privileges without needing a separate installer.
    os.rename('./airdesk_portable.exe', './airdesk-qs.exe')
    print(
        f'output location: {os.path.abspath(os.curdir)}/airdesk-qs.exe')
